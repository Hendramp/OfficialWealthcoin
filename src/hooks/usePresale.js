...
export function usePresale(account, chainId, providerSource, addToast) {
  ...
  const refresh = useCallback(async () => {
    try {
      ...
      const [rate, totalSold, totalRaised, remainingTokens, currentStage, currentStageRemaining, started, paused, startTime, presaleCap, totalStaked] = await Promise.all([
        presale.getCurrentRate(),
        presale.totalSold(),
        presale.totalRaised(),
        presale.remainingTokens(),
        presale.currentStage(),
        presale.currentStageRemaining(),
        presale.hasStarted(),
        presale.paused(),
        presale.startTime(),
        presale.PRESALE_CAP(),
      ...
      setStats({ rate, totalSold, totalRaised, remainingTokens, currentStage, currentStageRemaining, started, paused, startTime, presaleCap, tokenBalance, polBalance, stakedBalance, pendingRewards, totalStaked });
    } catch (error) {
      console.error(error);
  ...
  }, [polInput, readProvider, stats.rate]);
